...
# 		params[:solver] = ADMM
# 		params[:reg] = [TVRegularization(10.0^log_lambda; shape=(nx, n_species*ny))]
...
begin
	lambda_compressed_sensing = 10.0^-2
	local params = Dict{Symbol, Any}()
	params[:reco] = "standard"
	params[:reconSize] = (nx, n_species*ny)
	params[:encodingOps] = [E_fi_us]
	params[:solver] = ADMM
	params[:reg] = [TVRegularization(lambda_compressed_sensing; shape=(nx, n_species*ny))]
	params[:iterations] = 30
	params[:ρ] = 0.1
